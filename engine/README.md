...
`rule-tournament` 支持 `rule-fast`、`rule-ev` 和 `rule-nn` 的任意两两组合。每个 block 运行 6 局平衡座位分配，并按 block bootstrap 置信区间（以整个 block 为重采样单位）。选择 `rule-nn` 时，必须用对应侧的 `--*-nn-model` 参数提供模型路径。
